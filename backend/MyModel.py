...
# Hopenet包装器
# 功能: 加载预训练的Hopenet模型（权重文件为hopenet_robust_alpha1.pkl），并冻结参数，仅用于推理。
# 同时，它将输出转换为角度值。
class msedmodel(nn.Module):
    def __init__(self):
        super(msedmodel, self).__init__()
        ...
        pretrained_dict = torch.load('D:/多媒体/一般场景人脸识别-第1组/face-recognition-system/backend/models/hopenet_robust_alpha1.pkl')
        model_dict = self.hope_model.state_dict()
        # 1. filter out unnecessary keys
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
        # 2. overwrite entries in the existing state dict
